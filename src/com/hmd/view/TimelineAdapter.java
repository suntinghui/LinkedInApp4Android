...
public class TimelineAdapter extends BaseAdapter {
	...
	public void setList(List<Status> weiboList){
		listStatus = weiboList;
	}
	...
	public View getView(final int position, View itemView, ViewGroup viewGroup) {// return a weibo item view
		...
		if (user.getGender().equalsIgnoreCase("f")) {// female
			...
			holder.iv_weiboitem_gender.setImageResource(R.drawable.img_weibo_userinfo_female_1);
		} else if (user.getGender().equalsIgnoreCase("m")) {
			holder.tv_weiboitem_location.setVisibility(View.VISIBLE);
			holder.iv_weiboitem_gender.setImageResource(R.drawable.img_weibo_userinfo_male_1);
		} else {// not know
		...
		if (sourceStatus == null) {// no source status --> original status
			...
			if (sourceurl == null || sourceurl.equalsIgnoreCase("")) {
				...
				ImageUtil.loadImage(R.drawable.img_weibo_item_pic_loading, sourceurl, holder.iv_weiboitem_statusImage);
			}
			...
			if (sourceurl == null || sourceurl.equalsIgnoreCase("")) {
				...
				ImageUtil.loadImage(R.drawable.img_weibo_item_pic_loading, sourceurl, holder.iv_weiboitem_sourceImage);
			}
		}
		
		ImageUtil.loadImage(R.drawable.img_weibo_userinfo_image_frame, status.getUser().getProfileImageUrl(), holder.iv_weiboitem_head);
